...
resource "azurerm_mssql_server" "primary" {
  ...
  tags                          = merge({ "Name" = "${var.sqlserver_name}-primary" }, var.tags, )
...
resource "azurerm_mssql_server" "secondary" {
  ...
  administrator_login          = var.admin_username == null ? "sqladmin" : var.admin_username
  administrator_login_password = var.admin_password == null ? random_password.main.result : var.admin_password
  tags                         = merge({ "Name" = "${var.sqlserver_name}-secondary" }, var.tags, )
...
resource "azurerm_key_vault_secret" "sqlpassword" {
  for_each                     = {for idx, database in tolist(var.database_name): idx => database  }
  name                         = "${replace(each.value, "_" , "-")}-sqlpassword"
  value                        = var.admin_password == null ? element(concat(random_password.main.*.result, [""]), 0) : var.admin_password
...
resource "azurerm_mssql_database" "db" {
  ...
  tags                            = merge({ "Name" = "${each.value}-primary" }, var.tags, )
...
resource "azurerm_mssql_database_extended_auditing_policy" "primary" {
  ...
  database_id                             = azurerm_mssql_database.db[each.key].id
  storage_endpoint                        = azurerm_storage_account.storeacc.0.primary_blob_endpoint
  storage_account_access_key              = azurerm_storage_account.storeacc.0.primary_access_key
  storage_account_access_key_is_secondary = false
  retention_in_days                       = var.log_retention_days
...
#     command = "sqlcmd -I -U ${azurerm_mssql_server.primary.administrator_login} -P ${azurerm_mssql_server.primary.administrator_login_password} -S ${azurerm_mssql_server.primary.fully_qualified_domain_name} -d ${azurerm_mssql_database.db[each.key].name} -i ${var.sqldb_init_script_file} -o ${format("%s.log", replace(var.sqldb_init_script_file, "/.sql/", ""))}"
#   }
# }
...
#   resource_group_name = local.resource_group_name
#   server_name         = azurerm_mssql_server.primary.name
#   databases           = [azurerm_mssql_database.db[each.key]].id
...
#   name                       = lower("extaudit-${each.value}-diag")
#   target_resource_id         = azurerm_mssql_database.db[each.key].id
